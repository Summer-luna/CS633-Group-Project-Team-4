...
export const AttendanceReport = () => {
  ...
  const createCsvHeader = () => {
    ...
    Object.keys(uniqueDate).sort((a,b) => {
  ...
  const createCsvData = () => {
    ...
    const uniqueDate = Array.from(
      new Set(rows.map((row) => row.date.substring(0,10)))
    ).sort();

    const result = Object.fromEntries(
      Array.from(transformedData, ([user, dateMap]) => [
        user,
        Object.fromEntries(
          uniqueDate.map((date) => [date, dateMap.get(date)])
        ),
      ])
    );
    ...
    const getAttendances = async () => {
      ...
      if (res.data) {
        ...
        const data = attendances.map((attendance) => {
          return { id: attendance.buID, name: attendance.fullName, firstName: attendance.firstName, lastName:attendance.lastName, date: attendance.created, attendance: attendance.attendanceType, attendanceId: attendance.id, userId: attendance.userId };
        });
        setRows(data);
      }
    };
